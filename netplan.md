...
本文档用于为 `ammm56/amcontrol` 项目规划一套与当前仓库架构一致的 3D 虚拟设备接入方案。目标是在**不破坏现有 WPF 工业控制主程序架构**的前提下，为 `VirtualMotionCardService` 增加一个可供 Vue3 + Three.js 外部 3D 仿真前端实时联动的 .NET 侧集成层。
