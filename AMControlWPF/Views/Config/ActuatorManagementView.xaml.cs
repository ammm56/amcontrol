...
namespace AMControlWPF.Views.Config
{
    public partial class ActuatorManagementView : UserControl
    {
        ...
        private void RefreshDetailPanel()
        {
            ...
            // 类型徽章
            var typeBadgeColor = GetTypeColor(item.ActuatorType);
        ...
        private Brush GetTypeColor(string actuatorType)
        {
            switch (actuatorType)
            {
                case "Cylinder": return TryFindResource("PrimaryBrush") as Brush;
                case "Vacuum": return new SolidColorBrush((Color)ColorConverter.ConvertFromString("#03A9F4"));
                case "StackLight": return TryFindResource("WarningBrush") as Brush;
                case "Gripper": return new SolidColorBrush((Color)ColorConverter.ConvertFromString("#9C27B0"));
        ...
        private void ShowCylinderDetail(CylinderConfigEntity e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(e.Description)) AddShield("描述", e.Description, "#455A64");
            if (!string.IsNullOrWhiteSpace(e.Remark)) AddShield("备注", e.Remark, "#607D8B");
        }
        ...
        /// <summary>
        /// 添加一行 hc:Shield 元数据标签（与轴参数配置右侧详情风格一致）
        /// </summary>
