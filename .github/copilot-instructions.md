...
- 用户偏好遵循当前整体设计做减法：`OkLogOnly(...)` 已经通过消息总线写日志，如需同时弹提示框直接改 `Ok()`，不要为此增加复杂封装；
...
- 在后续协作中，先检查项目结构、分层架构、导航层级、页面布局与实现进度，再基于现有代码给出 WinForms 开发计划；当前优先在既有实现上收口 PLC / Motion / AlarmLog 页面，其次补齐首页与远期模块。

### 事件处理与日志管理

- 用户偏好在现有实现上做减法，避免为解决问题继续增加复杂字段或辅助逻辑；更倾向通过简化和统一返回消息来修复问题。
...
- `PlcDebugPage` 最终布局要求：严格复用现有页面三行结构；第一行与现有工具栏样式一致（左侧 PLC 下拉 + 状态，右侧刷新）；第二行留空占位；第三行按 `PLC配置` 页风格做 35%/65% 左右两列，左侧两个卡片（按配置点位调试、按地址读写），右侧为执行结果与历史操作记录 `Table`；所有调试读写操作都需通过消息总线写入日志。
