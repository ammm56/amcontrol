...
- **用户编辑对话框**：作为新增/编辑共用窗口时，新增用户模式应使用更大的窗口尺寸，编辑用户模式可使用更紧凑尺寸；同时应删除权限页与主窗口中的旧过渡传参代码。用户编辑对话框与重置用户密码对话框需要按 LoginForm 风格实现：纹理背景 + 大卡片；卡片分三行，顶部固定标题说明，中间可滚动表单区，底部固定右对齐按钮栏。
...
- 页面内校验/页面信息提示不优先使用 AntdUI.Message；系统消息通知与 reporter 已负责消息弹出。页面级信息更倾向使用 AntdUI.Modal 这类对话框，与系统通知链路区分。
- 涉及数据处理的成功/错误结果由系统消息总线统一负责展示，用户管理页这类页面不再额外弹出成功或失败对话框；页面只保留必要的交互确认类对话框。
...
- WinForms 用户管理页取消右侧详情栏，操作按钮放到搜索行下方，表格占满整行；明暗主题优先使用 AntdUI 默认样式并避免自定义导致样式异常；UserEditDialog 与 ResetUserPasswordDialog 布局需同步简化重做，按 LoginForm 风格实现：纹理背景 + 大卡片；卡片分三行，顶部固定标题说明，中间可滚动表单区，底部固定右对齐按钮栏。