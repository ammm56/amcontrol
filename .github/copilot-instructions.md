...
| **AM.Model** | 模型层 | Entity（Auth / Motion 拓扑 / Motion 轴参数 / Motion 执行器 / Motion 点位 / PLC / Dev 报警 / License / Device）、运行时 Config 对象（MotionCardConfig / AxisConfig / ActuatorConfig / PlcConfig）、接口定义（IMotionCardService / IPlcClient）、运行时状态缓存（MotionIoRuntimeState / MotionAxisRuntimeState / PlcRuntimeState）、公共结构体（AxisStatus / AxisParam）、统一 Result/Result\<T\> 返回模型 |
| **AM.Tools** | 工具层 | NLog 实现 NLogLogger、配置文件读写 Tools.ReadConfig/SaveConfig、MessageBus 实现、AppReporter 实现、错误码目录 JsonErrorCatalog |
| **AM.DBService** | 数据服务 | SqlSugar 数据库操作；Auth 服务（AuthService / AuthSeedService）；Motion 拓扑 CRUD（MotionCardCrudService / MotionAxisCrudService / MotionIoMapCrudService）；Motion 点位与轴参数（MotionIoPointConfigCrudService / MotionAxisConfigService / MotionAxisConfigOverlayService）；执行器 CRUD×4；PLC 配置/协议门面/运行时服务；授权与设备后台链路（LicenseRuntimeLoader / DeviceLicenseApplyClient / DeviceRegisterClient / DeviceHeartbeatClient / DeviceReportClient / UsageUploadWorker）；Dev 报警记录（DevAlarmRecordService）；运行时（IoScanWorker / MotionAxisScanWorker / PlcScanWorker / RuntimeTaskManager）；配置重载（MachineConfigReloadService / MachineConfigSeedService / PlcConfigAppService） |
...
| **AM.App** | 启动器 | AppBootstrap 组合根，统一编排启动顺序、启动期授权装载与后台工作单元注册 |
...
6. PlcConfigSeedService 初始化 PLC 配置种子数据
7. `ProtocolAssemblyRegistry.Reload()` 加载协议实现程序集
8. **MachineConfigReloadService.ReloadAndRebuild()** 从 DB 加载设备配置 → 重建 MachineContext（创建控制卡实例、轴/IO 映射、执行器注册）
9. `PlcConfigAppService.ReloadFromDatabase()` 从 DB 加载 PLC 配置 → 重建 PLC 客户端上下文
10. `LicenseRuntimeLoader.Load()` 启动期读取并校验本地 `license.lic`
11. 按 InitOrder 顺序 Initialize + Connect 所有控制卡
12. 注册 IoScanWorker（50ms IO 扫描）+ MotionAxisScanWorker（100ms 轴采样）+ PlcScanWorker（PLC 扫描）→ RuntimeTaskManager 管理启停
13. 注册可选后台任务 `UsageUploadWorker`，统一驱动设备注册 / 心跳 / report
14. Program.cs 主循环：`SyncPagePermissions()` → 登录 → `Application.Run(MainWindow)` → 按 ExitReason 决策（SwitchUser / Logout / Exit）
...
### 4.1 Auth 认证（6 张表，统一 `sys_*` 下划线命名）

- `sys_user` / SysUserEntity
- `sys_role` / SysRoleEntity
- `sys_user_role` / SysUserRoleEntity
- `sys_login_log` / SysLoginLogEntity
- `sys_page_permission` / SysPagePermissionEntity
- `sys_user_page_permission` / SysUserPagePermissionEntity
...
- Motion IO 点位公共配置采用“拓扑映射 + 点位公共配置”分层：拓扑由 `motion_io_map` 管理，显示名、取反、滤波、输出模式等公共属性由 `motion_io_point_config` 管理。
...
### 4.6 PLC（当前主实现为 2 张配置表）

- `plc_station` / PlcStationConfigEntity
- `plc_point` / PlcPointConfigEntity
- 当前代码未使用独立 `ReadBlock` 配置表，扫描粒度由站配置、点位配置和运行时服务共同决定
...
- PLC 点位当前保留 `PlcName`，用于与 `plc_station.Name` 形成稳定关联；
- 地址模型保持极简：使用单一 `Address` 直接表达协议地址；
- 长度字段统一为 `Length`，不再拆成 `StringLength` / `ArrayLength`；
...
- 支持寄存器内字节/位偏移：`40010.H`、`40010.L`、`40010.0`、`40010.1`；其中 `40010.0/1` 明确表示寄存器内 bit 量，不能再被识别成 `00010` 线圈。

---

## 五、导航体系

NavigationCatalog（AM.PageModel/Navigation）当前实际定义 11 个一级模块、30+ 个二级页面：

| 一级模块 | 二级页面 |
|----------|----------|
| Assembly 装配接线 | Wiring 接线与调试 |
| Home 首页 | Overview 总览看板、SysStatus 系统状态 |
| Motion 设备 | DI 监视、DO 监视、Monitor 多轴总览、Axis 轴控制、Actuator 执行器控制 |
| Production 生产 | Recipe、Data、Report |
| Vision 视觉 | Monitor、Result、Calibrate |
| PLC | Status 通讯状态、Monitor 点位监视、Debug 调试工具 |
...
- 对于后台扫描与类似高频后台任务的重复离线/重连等待/配置误触发信息，统一节流与静默控制，倾向通过 ServiceBase 或更合适的全局层实现 FailSilent、WarnLogOnly、WarnSilent 等能力，适配 PLC、IO、轴、相机、扫描器等全解决方案后台任务。
- 默认实现策略遵循当前整体设计做减法：`OkLogOnly(...)` 已经通过消息总线写日志，如需同时弹提示框直接改 `Ok()`，不要为此增加复杂封装；
...
- 在 WinForms 中用 AntdUI 实现报警抽屉面板，并在底部状态栏加入 Motion 连接状态、PLC 连接状态和报警按钮，风格参考现有 WPF 版本。
...
- PlcScanWorker：负责顶层调度，每个 PLC 站由 PlcStationScanRunner 独立执行连接维护与点位扫描
- UsageUploadWorker：后台统一驱动设备注册、心跳、report 与使用事件上传；失败消息按前缀压缩分类
...
- `IPlcScanWorker` 采用 `IPlcScanWorker : IRuntimeWorker` 的继承方式；PLC 扫描实现应遵循现有整体方案、避免多余封装；并要求为接口/实现中的每个属性、字段、方法补充详细中文注释说明。
...
- 当前仓库为“旧式 .NET Framework 项目 + SDK-style 项目”混合工程；VS2019 仅作为兼容构建/代码编辑环境，UI 设计器优先 VS2022
...
- 页面内校验/信息提示默认优先使用 AntdUI.Modal，对系统级成功/失败消息继续交由消息总线统一展示
- 数据处理成功/失败由系统消息总线统一展示，页面不额外弹成功/失败对话框
- 页面只保留必要的交互确认类对话框
...
| PlcConfigSeedService + PlcConfigAppService | ✅ |
| MotionServiceHub 统一路由调度 | ✅ |
| VirtualMotionCardService / GoogoMotionCardService / LeisaiMotionCardService | ✅ 已接入现有启动与运行链路 |
| RuntimeContext 运行时状态缓存（IO / 轴 / PLC） | ✅ |
| PLC 接口分层（IPlcClient / Config / Runtime） | ✅ |
| ProtocolAssemblyRegistry + ProtocolPlcClient + NullPlcClient | ✅ |
| PLC 驱动实现（ModbusTcp / S7Tcp） | ✅ |
| PlcScanWorker + PlcStationScanRunner | ✅ |
| 本地授权校验（license.lic） | ✅ |
| 设备授权申请 / 注册 / 心跳 / report | ✅ |
| DeviceRequestCryptoService（BouncyCastle HKDF + AES-GCM） | ✅ |
| 后端/PLC 失败前缀（BackendTimeout / BackendUnavailable / PlcDisconnected / ProtocolSocketError） | ✅ |
| 视觉模块 | ❌ 待实现 |
...
| Assembly.Wiring（装配接线页） | ✅ 首版 |
| UserManagementPage（搜索+统计+Table+CRUD） | ✅ |
| UserPermissionPage（权限分配） | ✅ 首版 |
| LoginLogPage（登录日志） | ✅ 首版 |
| UserEditDialog（新增/编辑共用 + LoginForm 风格） | ✅ |
| ResetUserPasswordDialog（LoginForm 风格） | ✅ |
| Motion.DI / Motion.DO 监视页 | ✅ 首版 |
| Motion.Monitor 多轴总览 | ✅ 首版 |
| Motion.Axis 轴控制 | ✅ 首版 |
| Motion.Actuator 执行器控制 | ✅ 首版 |
| MotionConfig.* 运控配置（5页） | ✅ 首版 |
| PLC.Status / PLC.Monitor / PLC.Debug | ✅ 首版 |
| SysConfig.Plc（PLC 配置） | ✅ 首版 |
| AlarmLog.Current / AlarmLog.History / AlarmLog.RunLog | ✅ 首版 |
| Home.Overview | ✅ 占位实现（2x2 图片轮播看板） |
| Home.SysStatus | ❌ Placeholder |
| Production.* | ❌ Placeholder |
| Vision.* | ❌ Placeholder |
| Peripheral.* | ❌ Placeholder |
| SysConfig.Camera / Sensor / Scanner / Mes / Runtime | ❌ Placeholder |
...
- 执行器相关模型类型默认使用 public，避免不必要的 internal 封装与过度抽象，保持贴合 WinForms 的直接实现。

### 事件处理风格

- 事件 lambda 默认统一写成 `(s, e) => ...` 风格，而不是 `(_, __) => ...`。

---

## 当前开发重点

- 后续协作默认先检查项目结构、分层架构、导航层级、页面布局与实现进度，再基于现有代码给出 WinForms 开发计划；当前阶段优先在既有实现上收口 PLC / Motion / AlarmLog 页面，其次补齐首页与远期模块。

### 实现与日志策略

- 默认实现策略是在现有实现上做减法，避免为解决问题继续增加复杂字段或辅助逻辑；优先通过简化和统一返回消息来修复问题。
- 后台运行类重复性离线/重连等待信息不应频繁弹消息或重复刷日志；此类信息应按状态变化或节流策略处理，避免日志风暴。

---

## PLC Debug Page 当前布局约束

- `PlcDebugPage` 当前布局按三行结构维护：第一行与现有工具栏样式一致（左侧 PLC 下拉 + 状态，右侧刷新）；第二行留空占位；第三行按 `PLC配置` 页风格做 35%/65% 左右两列，左侧两个卡片（按配置点位调试、按地址读写），右侧为执行结果与历史操作记录 `Table`；所有调试读写操作都需通过消息总线写入日志。

### 装配接线页交付约束

- 装配接线页不再区分“未定义”“已刷新”“异常项”；当前场景下 IO 数据存在即视为已定义，统计只保留已核对/未核对，接线状态只显示已核对/未核对，并应删除装配接线实现中的重复、冗余、未使用代码。
- 装配接线页表格应面向电工：不显示软件点名，优先展示逻辑IO号与硬件位号的对应关系，新增板载/扩展列；接线信息、对端设备、现场信息放到表格后部作为辅助说明。
- 装配接线页底部检查区应采用两列规范布局；脉冲宽度标签与输入框同一行；已核对/取消核对应尽量宽松，只要有运控配置中的 IO 定义即可操作，不依赖完整接线说明。
