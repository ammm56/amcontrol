...
5. **协议库已导入并完成基础验证**  
   旧项目中的 `ProtocolLib.ModbusTcp`、`ProtocolLib.S7Tcp` 已导入当前解决方案，且已补充基础集成测试：
   - 连接、重连、关闭；
   - 单点读写；
   - 常用基础类型读写；
   - `M_TypedValue` 比较与基本运算；
   - 固定长度字符串 `[20]` 方案；
   - 协议层 `Modbus` / `S7` 基本能力已达到可接入 AM 运行时的水平。

6. **当前阶段进入“协议库统一接口 + AM 接入”**  
   现阶段不再继续把 PLC 业务逻辑下沉到 `AM.DBService` 的协议细节实现中，而是进入下一阶段：
   - 在 `ProtocolLib` 内补齐统一的点位/块读写接口；
   - `AM` 上层仅通过协议库公开接口调用；
   - `MachineContext` 中保留 `IPlcClient` 作为系统统一入口，但该入口仅是薄门面，不直接操作协议底层客户端。
...
> 2026-04-10 设计调整：
>
> - 不再推荐 `AM.DBService` 直接操作 `ModbusTCP` / `SiemensS7` 等协议底层客户端；
> - 读写能力应尽量收敛在 `ProtocolLib` 内部实现；
> - `AM` 上层只通过协议库公开统一接口使用 PLC 能力；
> - `IPlcClient` 仍保留，但仅作为 `AM` 侧门面与生命周期入口，不再承担协议实现。
...
- 建立连接、断开连接；
- 查询连接状态；
- 对 AM 运行层暴露点位读写与块读写统一入口；
- 将 `Result/Result<T>` 与协议库 `M_Return<T>` 做边界转换；
- 通过反射创建具体协议库实现；
- 不直接触碰 `ModbusTCP`、`SiemensS7` 等底层客户端类。
...
- `Result<PlcPointReadResult> ReadPoint(...)`
- `Result<PlcPointReadResult> WritePoint(...)`
- `Result<PlcRawDataBlock> ReadBlock(...)`
- `Result<PlcRawDataBlock> WriteBlock(...)`

说明：

- 上层默认按点位读写；
- 仅在明确连续地址、区域一致、类型一致且已配置读块时才走块读写；
- `IPlcClient` 不是协议实现类，而是 `AM` 访问协议库的统一门面。

## 8.2 协议库统一公开接口

建议文件：
- `ProtocolLib/Common/CommonLib/Interface/IProtocol.cs`
...
- 定义协议库统一点位读写接口；
- 定义协议库统一块读写接口；
- 屏蔽 `Modbus`、`S7` 等协议地址、长度、位索引、字符串长度等差异。
...
- 当前 `Init / Get / Set / SetCFG / CloseConnected` 仅作为过渡接口保留；
- 最终公开接口应统一命名与结构化请求；
- 详细接口草案见 `Docs/01-architecture/plc-protocol-integration-design.md`。
...
- 按 `ProtocolType + ConnectionType + Vendor + Model` 解析协议类型；
- 由 `AM` 上层创建统一门面客户端；
- 门面客户端内部再通过反射创建具体协议库 `IProtocol` 实例；
- 不在页面、ViewModel、运行服务中直接 `new` 具体协议实现。
...
AM.DBService/Services/Plc/Driver/
  ...
  PlcClientFactory.cs
  PlcProtocolResolver.cs
```

建议实现顺序：

1. `ProtocolLib.CommonLib.Interface.IProtocol` 扩展统一接口
2. `ProtocolLib.ModbusTcp.Protocol` 补齐点位/块读写
3. `ProtocolLib.S7Tcp.Protocol` 补齐点位/块读写
4. `AM.DBService` 中落地统一门面 `ProtocolPlcClient`
5. `PlcClientFactory` 与 `PlcScanWorker` / `PlcOperationService` 接入新接口

理由：

- 协议行为应先在协议库内部收敛，不能继续下沉到 `AM.DBService` 自己实现；
- `ModbusTcp` 可作为第一条打通链路；
- `S7` 在统一接口稳定后跟进；
- `AM` 上层只保留薄门面与生命周期管理，避免协议细节扩散。
...
- 工业现场首版实现建议按“单站异常不拖垮整轮扫描”处理：某一 PLC 断线或局部读块失败时，优先更新该站与相关点位的运行态质量，而不是立即中断全部 PLC 扫描循环。

### 9.2.4 `PlcOperationService`

补充要求：

- 点位读写必须优先走 `ReadPoint / WritePoint`；
- 只有调试寄存器页、连续块配置扫描、明确的批量场景才调用 `ReadBlock / WriteBlock`；
- 上层不应自行拼接 `40040[20]`、`40001.2` 等协议表达式；
- `stringLength`、`bitIndex`、`arrayLength` 等结构化字段由协议库内部解释并转换为协议表达式。
...
- 除 `AM.Model` 侧的 `IPlcClient`、`PlcRawDataBlock` 等对象外，协议库本身需要新增统一请求/结果模型；
- 协议接口草案与最终签名设计见 `Docs/01-architecture/plc-protocol-integration-design.md`；
- 本规划文档保留模块级职责与落位，详细接口定义以后者为准。
