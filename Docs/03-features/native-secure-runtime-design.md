...
本文档原用于整理设备授权申请、设备注册、设备心跳、设备信息上报相关敏感材料的本地保护改造方案，并将前期讨论沉淀为一套 `Native` 安全模块规划。
...
4. 当前设备后台链路已通过 `BouncyCastle + AES-GCM + 长期 DeviceToken` 稳定运行；
5. 本文档仅保留为后续恢复 native 安全模块方案时的历史规划基线。

因此，本文档的使用边界如下：

1. 用于理解为何曾规划 `Native` 安全模块；
2. 用于对照当前“代码内置敏感默认值”方案与更强本地隔离方案的差异；
3. 不作为当前设备授权/设备上报实现依据；
4. 当前实现应优先参考 [device-management-runtime-flow.md](device-management-runtime-flow.md)、[device-license-and-reporting.md](device-license-and-reporting.md) 和 [software-license-design.md](software-license-design.md)。
...
以下内容保留的是当时规划 `Native` 安全模块时的设计约束，而非当前仓库已落地事实：
