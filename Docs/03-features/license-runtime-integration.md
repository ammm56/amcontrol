...
1. `LicenseRuntimeLoader` 必须执行正式 `RSA-SHA256` 验签；
2. `LicenseCryptoService` 只从程序内置 PEM 公钥资源读取验签公钥；
3. 首版不实现外部公钥文件或多 KeyId 路由；
4. 拦截性强校验字段固定为 `ClientId`、`MachineCode`、`CpuId`；
5. 其余硬件字段只做采集和日志诊断，不影响 `IsValid` 判定。

时间处理口径固定为：

1. 运行时内部对 `IssueTime`、`NotBefore`、`ExpiresAt` 的有效期判断继续按 UTC 语义处理；
2. WinForms 许可证显示控件仅在页面层将这些时间转换为本地时间输出；
3. 页面展示层的本地时间转换不参与授权状态计算。
