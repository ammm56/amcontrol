...
本文档用于沉淀当前 `AMControlWinF` 已落地页面的 UI 基线，作为当前 WinForms 分支内新页面与旧页面收口的参考。

目标不是把当前实现上升为长期永久规范，而是从当前 WinForms 代码中提炼出**可复用的、与现有实现一致的分支级基线**。

本文档的作用边界如下：

1. 适用于当前 `AMControlWinF` 分支内的页面新增、收口和局部重构；
2. 用于减少当前 WinForms 分支内部的样式和交互分化；
3. 不用于否定未来新的 UI 主线、跨平台前端或更大范围的界面重构；
4. 若后续仓库新增新的 UI 主线，应为新主线建立自己的界面基线文档。
...
## 4. 当前 WinForms 生命周期基线

### 4.1 缓存页规则

对于当前被 `MainWindow` 缓存复用的页面，默认遵循以下做法：
...
- 当前 WinForms 分支中，高频刷新默认只用于运行监视页；
- 搜索输入、筛选切换时默认避免刷新打断用户操作；
- 当前实现中，运行时变化默认进入 `RuntimeContext`，UI 自行低频拉取。
...
在当前 WinForms 分支中，页面按钮默认还要在自身内部再做一次权限态控制，不能只依赖导航是否可见。
...
当前 WinForms 分支内的新页面，默认至少满足以下 DoD：
...
当前 WinForms 页面体系已经具备较稳定的 UI 基线。对当前 WinForms 分支内的增量开发，默认以**收口现有页面**和**按模板补齐新页面**为主；若后续仓库引入新的 UI 主线或进行更大范围界面重构，应新增对应文档，而不是继续把本页当作永久规则使用。
